...
## POST /v1/files/batch\_upload/\{batchJobId}:complete

API endpoint for POST requests to /v1/files/batch\_upload/\{batchJobId}:complete.

```
Method: POST
Path: /v1/files/batch_upload/{batchJobId}:complete
```
...
API endpoint for GET requests to /v1/files/\{fileId}.
...
API endpoint for DELETE requests to /v1/files/\{fileId}.
...
API endpoint for PUT requests to /v1/files/\{fileId}.
...
|------|---------------------|-------------|
| **[Web Search](/docs/guides/tools/search-tools)** | $10 | Internet search and page browsing |
| **[X Search](/docs/guides/tools/search-tools)** | $10 | X posts, users, and threads |
| **[Code Execution](/docs/guides/tools/code-execution-tool)** | $10 | Python code execution environment |
| **[Document Search](/docs/guides/files)** | $10 | Search through uploaded files and documents |
...
Adding them in the request would result in error.
...
* **Model**: Use reasoning models like `grok-4-fast` for better code generation
...
async def main():
    ...
    chat = client.chat.create(
        model="grok-4-fast",  # Use a reasoning model for better analysis
        tools=[
            collections_search(
...
* **Supported Models**: `grok-4`, `grok-4-fast`, `grok-4-fast-non-reasoning`
* **Strongly Recommended**: `grok-4-fast` - specifically trained to excel at agentic tool calling
...
If you want to use monthly billing, you can **increase your invoiced billing limit** on [Billing -> API Credits](https://console.x.ai/team/default/billing) page. xAI will attepmt to use your prepaid credits first, and the remaining amount will be charged to your default payment method at the end of the month. This ensures you won't experience interruption in consuming the API.
...
You can add/remove your sign-in methods at https://accounts.x.ai. Your account must have at least one sign-in methods.
